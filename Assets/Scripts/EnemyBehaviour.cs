...
public class EnemyBehaviour : MonoBehaviour {
	...
	public bool PlayerInSight(GameObject player, GameObject npc)
	{
		...
		{
			RaycastHit hit;
			if (Physics.Raycast(npc.transform.position,toPlayer, out hit, sightRange))
			{
				return true;
			}
...
public class PatrolState : FSMState
{
	...
	{
		EnemyBehaviour behaviour = npc.GetComponent<EnemyBehaviour>();
        //Check line of sight.
        if (behaviour.PlayerInSight (player, npc)) {
			//TODO: Make a transition using Transition.SawPlayer.
			behaviour.fsm.PerformTransition(Transition.SawPlayer);
		}
	}
	...
	public override void Act(GameObject player, GameObject npc)
	{
		...
		float distance = Vector3.Distance(npc.transform.position, waypoints[currentWayPoint].transform.position);
...
public class ChasePlayerState : FSMState
{
	...
	public override void Reason(GameObject player, GameObject npc)
	{
        ...
        if (!behaviour.PlayerInSight (player, npc)) {
            ...
            behaviour.fsm.PerformTransition(Transition.LostPlayer);
